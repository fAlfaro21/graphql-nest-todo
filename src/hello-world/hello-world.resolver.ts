...
export class HelloWorldResolver {
    ...
    //OJO: Hay que utilizar la importación 'Query' de graphql porque hay otro que no nos sirve
    ...
    @Query( () => Float, { name: 'randomNumber' } ) //El float se importa de grapql
    ...
    getRandomFromZeroTo( 
        @Args('to', { //'to' es el nombre del argumento para GQL
            nullable: true, //Le decimos que puede ir vacío...por defecto pondermos un 6
            type: () => Int //Con el type Int nos aseguramos de que el argumento sea un entero 
            }
        ) 
        upTo: number = 6 //upTo es el nombre para mi
    ): number {  
